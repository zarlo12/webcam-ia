...
export type SummitFilterId = 1 | 2 | 3 | 4;

/** Colección de Firestore. No la comparte ninguna otra campaña. */
export const SUMMIT_COLLECTION = "claro_tech_summit_participantes";

/** Carpetas de Storage, todas bajo el mismo prefijo. */
export const SUMMIT_STORAGE = {
...
export interface SummitFilter {
  id: SummitFilterId;
  /** Nombre visible en la pantalla de estilos. */
  label: string;
  /**
   * Archivo dentro de backend/functions/assets/summit/. El backend lo publica
   * en Storage la primera vez que se usa el filtro, así que no hay que subir
   * nada a mano al desplegar.
   */
  referenceFile: string;
  prompt: string;
}

export const SUMMIT_FILTERS: Record<SummitFilterId, SummitFilter> = {
  1: {
    id: 1,
    label: "Acuarela",
...
export const isSummitFilterId = (value: unknown): value is SummitFilterId =>
