...
const AvatarPhoto: React.FC<AvatarPhotoProps> = ({ onProcess, onAiImageReady }) => {
  ...
  const getPromptByGender = (gender: Gender): string => {
    switch (gender) {
      case Gender.MALE:
        return "Edit ONLY the clothing and background. ABSOLUTELY DO NOT MODIFY THE FACE. The man's face must remain 100% IDENTICAL to the original photo at the pixel level. Do NOT change: - face shape - eyes - nose - mouth - eyebrows - jaw - cheekbones - ears - hairline - hair - facial hair - skin tone - age - expression - lighting on the face NO facial enhancement, NO beauty filter, NO retouching, NO stylization, NO face reconstruction. ONLY change the outfit to an ornate red and gold Nutcracker soldier uniform with gold buttons, shoulder epaulettes, and a tall red shako hat with gold trim and a white plume. The FACE MUST REMAIN EXACTLY THE SAME AS THE INPUT IMAGE. Photorealistic, identity must be perfectly preserved. "
      case Gender.FEMALE:
        return "Edit ONLY the clothing, pose, and background. ABSOLUTELY DO NOT MODIFY THE FACE. The woman's face must remain 100% IDENTICAL to the original photo at the pixel level. Do NOT change: - face shape - eyes - nose - mouth - eyebrows - jaw - cheekbones - ears - hairline - hair - facial proportions - skin tone - age - smile - expression - lighting on the face NO facial enhancement, NO beauty filter, NO retouching, NO stylization, NO face reconstruction. ONLY change the outfit to a Nutcracker ballerina costume: red satin ballet bodice with gold embroidery, white tulle tutu, and pointe shoes. The body should be posed gracefully in a ballet pose while standing on pointe. Add a warm festive Christmas background with soft glowing lights and cinematic studio lighting, realistic shadows, and shallow depth of field. The FACE MUST REMAIN EXACTLY THE SAME AS THE INPUT IMAGE. Photorealistic, identity must be perfectly preserved. ";
      default:
        return "";
    }
  };
