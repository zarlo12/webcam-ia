...
import circusReplicateService from "../services/circusReplicateService";
 ...
 */
export const generateCircusImage = onRequest(
  {
    cors: true,
 ...
 */
const handleCircusMultipartRequest = async (
  req: any,
  res: any,
): Promise<void> => {
  try {
 ...
 */
async function processCircusMultipartBody(
  body: Buffer,
  boundary: string,
  res: any,
) {
  ...
  const result =
    await circusReplicateService.generateCircusTransformation(request);
 ...
 */
export const circusHealthCheck = https.onRequest(
  {
    cors: true,
    maxInstances: 10,
    timeoutSeconds: 30,
 ...
 */
export const getCircusStatus = https.onRequest(
  {
    cors: true,
    maxInstances: 10,
    timeoutSeconds: 60,
  ...
  async (req, res) => {
    try {
      ...
      const status = await circusReplicateService.checkStatus(predictionId);
      res.status(200).json({
        success: true,
        data: status,
      });
